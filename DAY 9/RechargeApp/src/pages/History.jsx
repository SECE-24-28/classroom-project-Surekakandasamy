...
export default function History() {
  ...
  useEffect(() => {
    console.log('History: Component mounted, loading all transactions');
    loadTransactions();
  }, []);

  // Get user-specific transactions
  const userTransactions = user?.email ? getUserTransactions(user.email) : [];
  console.log('History: User transactions:', userTransactions);

  const filteredTransactions = filter === 'all' 
    ? userTransactions 
    : userTransactions.filter(t => t.status && t.status.toLowerCase() === filter);
