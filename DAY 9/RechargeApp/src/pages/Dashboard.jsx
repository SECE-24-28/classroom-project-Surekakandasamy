...
export default function Dashboard() {
  ...
  const { transactions, loadTransactions, getUserTransactions } = useTransactions();
  const navigate = useNavigate();
  
  const recentTransactions = userTransactions.slice(0, 3);
  const activePlans = userTransactions.length;
  const currentMonth = new Date().getMonth();
  const thisMonthAmount = userTransactions
    .filter(t => t.date && new Date(t.date).getMonth() === currentMonth)
  ...
  useEffect(() => {
    if (!isLoggedIn) {
      ...
      console.log('Dashboard: Loading all transactions');
      loadTransactions();
    }
  }, [isLoggedIn, navigate, loadTransactions]);

  // Get user-specific transactions
  const userTransactions = user?.email ? getUserTransactions(user.email) : [];
  console.log('Dashboard: User transactions:', userTransactions);
